...
for (i in seq_len(nrow(param_grid))) {
    ...
    if (identical(ribocond, rnacond)) {
        # Run RiboDiff
        system("PYTHONPATH=../dotseq/src/RiboDiff/src /opt/miniconda2/bin/python /opt/RiboDiff/scripts/TE.py -e data/simdata/exp_outline.txt -c data/simdata/counts.txt -o results/benchmarking/ribodiff.out")
        ...
        res_xt <- resultsTable(res_xt$xtail)
        end_xt <- Sys.time()
    ...
    padj_df[rownames(res_rrx$res), "Riborex:padj"] <- res_rrx$res$padj
